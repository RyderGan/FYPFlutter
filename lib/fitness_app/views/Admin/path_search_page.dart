...
class _PathSearchPageState extends State<PathSearchPage> {
  List<PathModel> allPathList = <PathModel>[].obs;
  List<PathModel> currentPathList = <PathModel>[].obs;

  Future getAllPaths() async {
    try {
  ...
  void _onQueryChanged() {
    ...
    for (final path in allPathList) {
      final name = path.name.toLowerCase();
      final location = path.points.toString().toLowerCase();
    ...
    for (final path in allPathList) {
      ...
      if (contains && !currentPathList.contains(path)) {
        currentPathList.add(path);
      }
    }
  ...
  Widget _buildItem(PathModel path) {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: Future.wait([getAllPaths()]),
        builder: (context, constraints) {
          return Scaffold(
            appBar: _buildAppBar(padding, theme, textTheme),
            body: AnimatedSwitcher(
              duration: const Duration(milliseconds: 300),
              child: currentPathList.isNotEmpty
                  ? _buildList()
                  : _buildNoPathModelsPlaceholder(),
            ),
          );
        });
  }
  ...
  Widget _buildList() {
    return ImplicitlyAnimatedList<PathModel>(
      items: currentPathList,
      updateDuration: const Duration(milliseconds: 400),
      areItemsTheSame: (a, b) => a == b,
      itemBuilder: (context, animation, path, _) {
        return SizeFadeTransition(
          sizeFraction: 0.7,
          curve: Curves.easeInOut,
          animation: animation,
          child: _buildItem(path),
        );
      },
      updateItemBuilder: (context, animation, path) {
  ...
  Widget _buildNoPathModelsPlaceholder() {
    return Center(
      child: Column(
